...
class BackendIMAP extends BackendDiff implements ISearchProvider {
    ...
    static $myclient;
    static $mylast; // last time we opened the connection
    ...
    public function ChangesSink($timeout = 30) {
        ...
        if (empty($notifications)) {
            ...
            ZLog::Write(LOGLEVEL_INFO, sprintf("BackendIMAP->ChangesSink(): Login so we can use idle"));
            ...
            if (!self::$myclient) {
                ...
                sleep($timeout);
            } else if (myidle(self::$myclient,$imapid,$timeout)) {
                $notifications[] = $this->getFolderIdFromImapId($imapid);                
